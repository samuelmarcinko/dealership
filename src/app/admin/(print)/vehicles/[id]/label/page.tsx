import { notFound } from 'next/navigation'
import { prisma } from '@/lib/prisma'
import { getTenantBranding } from '@/lib/tenant'
import { CATEGORY_META, CATEGORY_ORDER } from '@/lib/equipmentData'
import PrintButton from '@/components/admin/PrintButton'

const FUEL_LABELS: Record<string, string> = {
  PETROL: 'Benzín',
  DIESEL: 'Diesel',
  ELECTRIC: 'Elektrický',
  HYBRID: 'Hybrid',
  LPG: 'LPG',
  CNG: 'CNG',
}

const TRANSMISSION_LABELS: Record<string, string> = {
  MANUAL: 'Manuálna',
  AUTOMATIC: 'Automatická',
  SEMI_AUTOMATIC: 'Poloautomatická',
}

const BODY_LABELS: Record<string, string> = {
  SEDAN: 'Sedan',
  HATCHBACK: 'Hatchback',
  ESTATE: 'Kombi',
  SUV: 'SUV',
  COUPE: 'Coupé',
  CONVERTIBLE: 'Kabriolet',
  VAN: 'Van',
  PICKUP: 'Pickup',
  OTHER: 'Iné',
}

function formatMileage(km: number) {
  return km.toLocaleString('sk-SK') + ' km'
}

function formatPrice(n: number) {
  return n.toLocaleString('sk-SK', { minimumFractionDigits: 0, maximumFractionDigits: 0 })
}

export default async function VehicleLabelPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params
  const [vehicle, branding] = await Promise.all([
    prisma.vehicle.findUnique({ where: { id } }),
    getTenantBranding(),
  ])

  if (!vehicle) notFound()

  const price = Number(vehicle.price)
  const salePrice = vehicle.salePrice ? Number(vehicle.salePrice) : null

  // Build equipment sections
  const featureMap: Record<string, string[]> = {
    safetyFeatures: vehicle.safetyFeatures,
    comfortFeatures: vehicle.comfortFeatures,
    multimediaFeatures: vehicle.multimediaFeatures,
    exteriorFeatures: vehicle.exteriorFeatures,
    otherFeatures: vehicle.otherFeatures,
    evFeatures: vehicle.evFeatures,
  }

  const categories = CATEGORY_ORDER
    .map((cat) => ({ cat, meta: CATEGORY_META[cat], items: featureMap[CATEGORY_META[cat].field] ?? [] }))
    .filter(({ items }) => items.length > 0)

  // Spec rows
  const specs: { icon: string; label: string; value: string }[] = []
  specs.push({ icon: 'calendar', label: 'Rok', value: String(vehicle.year) })
  specs.push({ icon: 'gauge', label: 'Najazdené', value: formatMileage(vehicle.mileage) })
  specs.push({ icon: 'fuel', label: 'Palivo', value: FUEL_LABELS[vehicle.fuelType] ?? vehicle.fuelType })
  specs.push({ icon: 'settings2', label: 'Prevodovka', value: TRANSMISSION_LABELS[vehicle.transmission] ?? vehicle.transmission })
  if (vehicle.bodyType) {
    specs.push({ icon: 'car', label: 'Karoséria', value: BODY_LABELS[vehicle.bodyType] ?? vehicle.bodyType })
  }
  const engineParts: string[] = []
  if (vehicle.engineCapacity) engineParts.push((vehicle.engineCapacity / 1000).toFixed(1) + 'l')
  if (vehicle.power) engineParts.push(vehicle.power + ' kW')
  if (engineParts.length > 0) {
    specs.push({ icon: 'cog', label: 'Motor', value: engineParts.join(' · ') })
  }
  if (vehicle.color) specs.push({ icon: 'palette', label: 'Farba', value: vehicle.color })
  const doorSeatParts: string[] = []
  if (vehicle.doors) doorSeatParts.push(vehicle.doors + ' dvere')
  if (vehicle.seats) doorSeatParts.push(vehicle.seats + ' miest')
  if (doorSeatParts.length > 0) {
    specs.push({ icon: 'door-open', label: 'Dvere/Miesta', value: doorSeatParts.join(' · ') })
  }
  if (vehicle.vin) specs.push({ icon: 'hash', label: 'VIN', value: vehicle.vin.slice(0, 17) })

  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&display=swap');
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        html, body { width: 297mm; height: 210mm; background: white; font-family: 'Inter', sans-serif; }
        @page { size: A4 landscape; margin: 8mm; }
        @media print {
          .no-print { display: none !important; }
          html, body { width: 297mm; height: 210mm; }
        }
        .label-root {
          width: 100%;
          height: 100%;
          display: flex;
          flex-direction: column;
          color: #1e293b;
          font-size: 11px;
          line-height: 1.4;
        }
        .header {
          display: flex;
          align-items: center;
          justify-content: space-between;
          padding: 6px 8px;
          border-bottom: 2px solid #f97316;
        }
        .dealer-name {
          font-size: 13px;
          font-weight: 700;
          color: #334155;
          letter-spacing: 0.02em;
        }
        .body {
          display: flex;
          flex: 1;
          overflow: hidden;
        }
        .col-identity {
          width: 35%;
          padding: 12px 14px;
          display: flex;
          flex-direction: column;
          justify-content: center;
          background: #fff;
        }
        .vehicle-title {
          font-size: 22px;
          font-weight: 800;
          color: #0f172a;
          line-height: 1.2;
          margin-bottom: 2px;
        }
        .vehicle-sub {
          font-size: 12px;
          color: #64748b;
          margin-bottom: 16px;
        }
        .price-original {
          font-size: 13px;
          color: #94a3b8;
          text-decoration: line-through;
          margin-bottom: 2px;
        }
        .price-main {
          font-size: 52px;
          font-weight: 900;
          color: #f97316;
          line-height: 1;
          letter-spacing: -0.02em;
        }
        .price-currency {
          font-size: 28px;
          font-weight: 700;
          vertical-align: top;
          line-height: 1.4;
        }
        .col-specs {
          width: 38%;
          padding: 12px 14px;
          border-left: 1px solid #e2e8f0;
          border-right: 1px solid #e2e8f0;
          display: flex;
          flex-direction: column;
          justify-content: center;
          gap: 5px;
        }
        .spec-row {
          display: flex;
          align-items: center;
          gap: 8px;
          font-size: 11px;
        }
        .spec-icon {
          color: #f97316;
          flex-shrink: 0;
          width: 16px;
          height: 16px;
        }
        .spec-label {
          color: #94a3b8;
          width: 70px;
          flex-shrink: 0;
          font-size: 10px;
        }
        .spec-value {
          color: #1e293b;
          font-weight: 500;
        }
        .col-features {
          width: 27%;
          padding: 10px 12px;
          display: flex;
          flex-direction: column;
          gap: 8px;
          overflow: hidden;
        }
        .feature-category {
          display: flex;
          flex-direction: column;
          gap: 2px;
        }
        .feature-cat-title {
          font-size: 9px;
          font-weight: 700;
          text-transform: uppercase;
          letter-spacing: 0.06em;
          color: #f97316;
          margin-bottom: 2px;
        }
        .feature-item {
          font-size: 10px;
          color: #475569;
          display: flex;
          align-items: flex-start;
          gap: 4px;
          line-height: 1.3;
        }
        .feature-check {
          color: #f97316;
          font-weight: 700;
          flex-shrink: 0;
        }
        .footer {
          border-top: 1px solid #e2e8f0;
          padding: 5px 8px;
          display: flex;
          align-items: center;
          justify-content: center;
          gap: 20px;
          font-size: 10px;
          color: #64748b;
          background: #f8fafc;
        }
        .footer-item {
          display: flex;
          align-items: center;
          gap: 5px;
        }
        .footer-icon {
          color: #f97316;
          width: 12px;
          height: 12px;
          flex-shrink: 0;
        }
      `}</style>

      <div className="label-root">
        {/* Header */}
        <div className="header">
          <span className="dealer-name">{branding.businessName}</span>
          <PrintButton />
        </div>

        {/* Body — 3 columns */}
        <div className="body">
          {/* Column 1 — Identity + Price */}
          <div className="col-identity">
            <div className="vehicle-title">{vehicle.title}</div>
            <div className="vehicle-sub">{vehicle.make} · {vehicle.model}{vehicle.variant ? ` · ${vehicle.variant}` : ''}</div>

            {salePrice ? (
              <>
                <div className="price-original">{formatPrice(price)} €</div>
                <div className="price-main">
                  <span className="price-currency">€</span>
                  {formatPrice(salePrice)}
                </div>
              </>
            ) : (
              <div className="price-main">
                <span className="price-currency">€</span>
                {formatPrice(price)}
              </div>
            )}
          </div>

          {/* Column 2 — Specs */}
          <div className="col-specs">
            {specs.map((s) => (
              <div key={s.label} className="spec-row">
                <SpecIcon name={s.icon} />
                <span className="spec-label">{s.label}</span>
                <span className="spec-value">{s.value}</span>
              </div>
            ))}
          </div>

          {/* Column 3 — Equipment */}
          <div className="col-features">
            {categories.map(({ cat, meta, items }) => (
              <div key={cat} className="feature-category">
                <div className="feature-cat-title">{meta.label}</div>
                {items.slice(0, 8).map((item) => (
                  <div key={item} className="feature-item">
                    <span className="feature-check">✓</span>
                    <span>{item}</span>
                  </div>
                ))}
                {items.length > 8 && (
                  <div className="feature-item" style={{ color: '#94a3b8', fontSize: '9px' }}>
                    + {items.length - 8} ďalších…
                  </div>
                )}
              </div>
            ))}
          </div>
        </div>

        {/* Footer */}
        <div className="footer">
          {branding.contactPhone && (
            <span className="footer-item">
              <PhoneIcon />
              {branding.contactPhone}
            </span>
          )}
          {branding.contactEmail && (
            <span className="footer-item">
              <MailIcon />
              {branding.contactEmail}
            </span>
          )}
          {branding.contactAddress && (
            <span className="footer-item">
              <MapPinIcon />
              {branding.contactAddress}
            </span>
          )}
        </div>
      </div>
    </>
  )
}

// Inline SVG icons — no additional deps needed
function SpecIcon({ name }: { name: string }) {
  const icons: Record<string, React.ReactNode> = {
    calendar: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>
      </svg>
    ),
    gauge: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M12 14l4-4"/><path d="M3.34 19a10 10 0 1 1 17.32 0"/>
      </svg>
    ),
    fuel: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <line x1="3" y1="22" x2="15" y2="22"/><line x1="4" y1="9" x2="14" y2="9"/><path d="M14 22V4a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v18"/><path d="M14 13h2a2 2 0 0 1 2 2v2a2 2 0 0 0 2 2h0a2 2 0 0 0 2-2V9.83a2 2 0 0 0-.59-1.42L18 5"/>
      </svg>
    ),
    settings2: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M20 7H9"/><path d="M14 17H3"/><circle cx="17" cy="17" r="3"/><circle cx="7" cy="7" r="3"/>
      </svg>
    ),
    car: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v9a2 2 0 0 1-2 2h-3"/><circle cx="7.5" cy="17.5" r="2.5"/><circle cx="17.5" cy="17.5" r="2.5"/>
      </svg>
    ),
    cog: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="12" cy="12" r="3"/><path d="M19.07 4.93l-1.41 1.41M4.93 4.93l1.41 1.41M12 2v2M12 20v2M20 12h2M2 12h2M19.07 19.07l-1.41-1.41M4.93 19.07l1.41-1.41"/>
      </svg>
    ),
    palette: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <circle cx="13.5" cy="6.5" r=".5" fill="currentColor"/><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"/><circle cx="8.5" cy="7.5" r=".5" fill="currentColor"/><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"/><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"/>
      </svg>
    ),
    'door-open': (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M13 4h3a2 2 0 0 1 2 2v14"/><path d="M2 20h3"/><path d="M13 20h9"/><path d="M10 12v.01"/><path d="M13 4.562v16.157a1 1 0 0 1-1.242.97L5 20V5.562a2 2 0 0 1 1.515-1.94l4-1A2 2 0 0 1 13 4.561z"/>
      </svg>
    ),
    hash: (
      <svg className="spec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <line x1="4" y1="9" x2="20" y2="9"/><line x1="4" y1="15" x2="20" y2="15"/><line x1="10" y1="3" x2="8" y2="21"/><line x1="16" y1="3" x2="14" y2="21"/>
      </svg>
    ),
  }
  return <>{icons[name] ?? null}</>
}

function PhoneIcon() {
  return (
    <svg className="footer-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.6 3.42 2 2 0 0 1 3.6 1.24h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.96a16 16 0 0 0 6.13 6.13l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/>
    </svg>
  )
}

function MailIcon() {
  return (
    <svg className="footer-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/>
    </svg>
  )
}

function MapPinIcon() {
  return (
    <svg className="footer-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/>
    </svg>
  )
}
